...
/** 단순 알림: 예약, 출퇴근, 연차 등 */
const SIMPLE_ENTITY_TYPES = ['reservation', 'attendance', 'work_request', 'leave'] as const;
/** 업무상 알림: 프로젝트 일정, 마감일, 할일, 댓글 등 */
const WORK_ENTITY_TYPES = ['task', 'project', 'comment'] as const;

function isSimpleNotification(entityType?: string): boolean {
  if (!entityType) return true;
  return SIMPLE_ENTITY_TYPES.includes(entityType as (typeof SIMPLE_ENTITY_TYPES)[number]);
}

function isWorkNotification(entityType?: string): boolean {
  if (!entityType) return false;
  return WORK_ENTITY_TYPES.includes(entityType as (typeof WORK_ENTITY_TYPES)[number]);
}
...
export function NotificationDrawer() {
  ...
  /** entity_type + entity_id → 이동 URL (특정 프로젝트/할일이면 해당 항목이 열리도록 id 포함) */
  const getTargetUrl = (notification: Notification): string | null => {
    ...
    const base = entityType ? baseMapping[entityType] ?? null : null;
    if (!base) return null;
    const openSpecific =
      entityId &&
    ...
    if (openSpecific) {
      ...
      return `${base}${sep}id=${encodeURIComponent(entityId)}`;
    }
    return base;
  };
  ...
  const { simpleNotifications, workNotifications, simpleUnreadCount, workUnreadCount } = useMemo(() => {
    const simple = notifications.filter(n => isSimpleNotification(n.entity_type));
    const work = notifications.filter(n => isWorkNotification(n.entity_type));
    return {
      simpleNotifications: simple,
      workNotifications: work,
      simpleUnreadCount: simple.filter(n => !n.read).length,
      workUnreadCount: work.filter(n => !n.read).length,
    };
  }, [notifications]);

  const hasAnyUnread = simpleUnreadCount > 0 || workUnreadCount > 0;
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <button
          ...
          aria-label="알림"
          title={hasAnyUnread ? `단순 알림 ${simpleUnreadCount}개, 업무 알림 ${workUnreadCount}개` : '알림'}
        >
          <Bell className="h-4 w-4 sm:h-3.5 sm:w-3.5 text-slate-600 dark:text-slate-300" />
          {hasAnyUnread && (
            <span className="absolute -top-0.5 -right-0.5 flex min-w-[36px] sm:min-w-[32px] h-5 sm:h-4 items-center justify-center gap-0.5 rounded-full bg-red-500 px-1 text-[10px] sm:text-[9px] font-bold text-white animate-pulse">
              <span>{simpleUnreadCount > 99 ? '99+' : simpleUnreadCount}</span>
              <span className="opacity-70">|</span>
              <span>{workUnreadCount > 99 ? '99+' : workUnreadCount}</span>
            </span>
          )}
        </button>
      </SheetTrigger>
      ...
      >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  {unreadCount > 0
                    ? `단순 ${simpleUnreadCount} · 업무 ${workUnreadCount} (읽지 않음)`
                    : '모든 알림을 읽었습니다'}
                </p>
              </div>
            </div>
        ...
        <Tabs defaultValue="simple" className="flex-1 flex flex-col min-h-0">
          <div className="px-4 sm:px-5 pt-2 border-b border-slate-200 dark:border-slate-700 shrink-0">
            <TabsList className="w-full grid grid-cols-2 h-10 bg-slate-100 dark:bg-slate-800 p-1">
              <TabsTrigger value="simple" className="gap-1.5 text-sm data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700">
                <Calendar className="h-4 w-4" />
                단순 알림
                {simpleUnreadCount > 0 && (
                  <span className="rounded-full bg-amber-500 text-white text-xs font-bold min-w-[18px] h-[18px] flex items-center justify-center px-1">
                    {simpleUnreadCount > 99 ? '99+' : simpleUnreadCount}
                  </span>
                )}
              </TabsTrigger>
              <TabsTrigger value="work" className="gap-1.5 text-sm data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700">
                <Briefcase className="h-4 w-4" />
                업무 알림
                {workUnreadCount > 0 && (
                  <span className="rounded-full bg-blue-500 text-white text-xs font-bold min-w-[18px] h-[18px] flex items-center justify-center px-1">
                    {workUnreadCount > 99 ? '99+' : workUnreadCount}
                  </span>
                )}
              </TabsTrigger>
            </TabsList>
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            {loading && notifications.length === 0 ? (
              ...
              <>
                <TabsContent value="simple" className="mt-0 flex-1 min-h-0">
                  <NotificationList
                    notifications={simpleNotifications}
                    emptyMessage="예약·출퇴근·연차 등 단순 알림이 없습니다"
                    onNotificationClick={handleNotificationClick}
                    onMarkAsRead={handleMarkAsRead}
                    onDelete={handleDelete}
                  />
                </TabsContent>
                <TabsContent value="work" className="mt-0 flex-1 min-h-0">
                  <NotificationList
                    notifications={workNotifications}
                    emptyMessage="프로젝트·할일·마감일 등 업무 알림이 없습니다"
                    onNotificationClick={handleNotificationClick}
                    onMarkAsRead={handleMarkAsRead}
                    onDelete={handleDelete}
                  />
