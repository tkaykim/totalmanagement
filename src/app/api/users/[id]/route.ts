...
import { canChangeUserRoleBuStatus, STAFF_ROLES } from '@/lib/permissions';
 ...
 * - 수정은 관리자만 한다(기존 동작 유지). 역할·사업부·재직 상태 변경은 `canChangeUserRoleBuStatus`로
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (appUser.role !== 'admin') return fail(403, '관리자만 수정할 수 있습니다.');
