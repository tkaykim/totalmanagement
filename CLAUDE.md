...
2026-09-24 대표 결정으로 이 ERP가 전사 프로젝트 키·재무 허브가 되었다. 그 전제 조건인 권한 봉인은 아직 되지 않았다.
...
│   ├── business-rules.md          → 사업부·프로젝트·할일·매출지출·세금·인건비·근태·휴가·예약 규칙(현재/목표)
│   ├── security.md                → 공개 저장소, 인증 흐름, 권한 표, RLS 현황, 감사 대상, 비밀값
│   ├── standards.md               → 커밋·배포·DB 변경·라우트·사업부 목록 규칙
│   ├── engineering-notes.md       → 함정(DB와 코드 시점 차이, 연쇄 삭제, 1,000행 절단, KST 등)
│   ├── operations.md              → 설치·명령어·환경변수·배포·DB 변경·모바일 빌드 절차
│   ├── contracts.md               → 워커·flowmaker·reactstudio가 기대하는 스키마, 크론, API 응답 형식
│   └── tracking/
│       ├── status.md              → 운영 중인 기능, 정리 대상, 남은 일, 대표 결정 대기
│       ├── findings.md            → 아직 못 고친 문제(치명·높음·보통)
│       └── decisions/
│           ├── index.md           → 결정 기록 목록
│           └── 0001~0010-*.md     → 허브화, 지급 분리, 급여, 일 합계, 내부배부, 외부 로그인, 공개 저장소, 재무 권한, 빈 지급 방식 해석, MODOO 주문 단위
├── src/
│   ├── app/api/AGENTS.md          → 서버 라우트: 인증·권한 확인 순서, update 입력, 크론
│   ├── lib/AGENTS.md              → Supabase 클라이언트 2종, permissions, 알림·푸시, KST
│   └── features/
│       ├── AGENTS.md              → 버그리포트·댓글·자료실·매뉴얼·업무일지·AI·전속 아티스트, 정리 대상 기능
│       ├── erp/AGENTS.md          → 프로젝트·할일·매출지출·조직 화면
...
├── supabase/AGENTS.md             → 마이그레이션(운영과 불일치)·Edge Function send-push
...
3. **`main`에 push하면 바로 운영 배포된다.** 승인 없이 `main`에 올리지 않는다. `npm run build`(타입 오류 = 실패)가 통과해야 한다.
4. **서버 권한 키(`createPureClient`)를 쓰는 코드는 쿼리 전에 세션·재직·역할을 확인한다.** update에 요청 본문을 통째로 넘기지 않는다.
5. **`paid`·`canceled` 매출·지출은 누구도 삭제하지 않는다.** 개인 급여를 ERP에 넣지 않는다(사업부별 월 합계 1행만).

## 작업 전 확인

- 항상 `docs/standards.md`, `docs/engineering-notes.md`, 그리고 손댈 폴더의 `AGENTS.md`를 먼저 읽는다.
- **권한·로그인·API 라우트를 건드릴 때**: `docs/security.md`의 권한 표와 RLS 현황, `src/app/api/AGENTS.md`의 확인 순서. 화면에서 막혀도 API·PostgREST로는 열려 있는 구조임을 전제로 한다.
- **매출·지출·손익 코드를 건드릴 때**: `docs/business-rules.md`의 2-4~2-7절(내부배부 제외, 금액 뜻 미확정, 수정 권한·삭제 금지, 인건비). 기존 행 금액을 보정하는 계산을 넣지 않는다.
- **스키마·enum·RLS를 바꿀 때**: `docs/contracts.md`의 외부 사용자 표, `supabase/AGENTS.md`, `docs/engineering-notes.md`의 "운영 DB와 코드가 서로 다른 시점" 항목. 스키마는 운영 DB를 직접 조회해 확인한다.
- **사업부 목록·DEETZ를 다룰 때**: 하드코딩된 6개 목록 30여 곳이 운영 DB의 7개와 다르다. 한 곳만 고치지 않는다.
- **날짜·크론을 다룰 때**: KST 헬퍼와 UTC 크론 스케줄.
- **대량 조회·집계**: 1,000행 절단.
- **아티스트·파트너 포털, 파트너 정산을 건드릴 때**: 정리 대상이다. 살리는 방향으로 고치지 않는다.
