...
[Next.js 앱 · Vercel 프로젝트 totalmanagement]
  ├─ 화면: `/`(메뉴 전환형 단일 대시보드) · /attendance · /leave · /artist · /login · /signup
  ├─ /api/* 라우트 140개 ── 쿠키 세션으로 사용자 확인 → 서비스 권한 키로 DB 접근
...
[Supabase wqtoahrekijirxxpbfqg]  [Gowid API]   [Firebase FCM]   [Gemini API]
  ...
  │                  /admin은 서비스 권한 키로 projects·project_tasks·financial_entries(REACT 지출)·app_users에 직접 씀
...
| 구성 요소 | 역할 | 의존 방향 |
|---|---|---|
| `src/app/page.tsx` | 로그인 후 메인 셸. 권한별 사이드바 메뉴를 만들고 선택한 기능 화면을 한 페이지 안에서 바꿔 끼움 | → `src/features/*` 화면, `src/lib/permissions` |
| `src/features/<기능>` | 기능별 화면·react-query 훅·`api.ts`(fetch 래퍼) | → `/api/*`(HTTP), 일부는 브라우저 Supabase 클라이언트로 DB 직접 조회 |
| `src/app/api/<자원>` | 서버 라우트. 세션 사용자 확인, 역할 판정, DB 읽기·쓰기, 활동 기록·알림 발송 | → `src/lib/supabase/server`, `src/lib/permissions`, `src/lib/notification-sender`, `src/lib/activity-logger` |
| `src/lib/supabase` | `createClient`(쿠키 세션, 익명 키)와 `createPureClient`(서비스 권한 키, 쿠키 없음) 두 가지 서버 클라이언트, 브라우저 클라이언트 1개 | → Supabase |
| `src/lib/permissions.ts` | 역할(admin·leader·manager·member·viewer·artist) × 사업부 기준 판정 함수 | 의존 없음(순수 함수) |
...
| `supabase/` | 레포에 남은 마이그레이션 31개와 Edge Function `send-push` 소스 | 운영 DB의 부분 기록일 뿐, 운영 스키마 전체를 담고 있지 않음 |
| `android/`, `ios/` | Capacitor 껍데기. 번들된 웹 자산 없이 운영 URL을 로드하고 푸시·위치·카메라 권한만 제공 | → 운영 웹앱 |

## 대표 흐름 — 매출·지출 한 건 등록

1. 직원이 프로젝트 상세에서 재무 입력 모달(`FinanceFormModals`)에 금액·지급 방식·지급처를 넣는다. 화면은 지급 방식에 따라 실지급액을 미리 계산해 보여 준다.
2. react-query 변이 훅이 `POST /api/financial-entries`를 호출한다.
3. 라우트가 쿠키 세션 클라이언트로 `auth.getUser()`를 확인하고, 서비스 권한 클라이언트로 `app_users`에 같은 id가 있는지만 본다. 역할·사업부·재직 상태는 보지 않는다.
4. 허용된 필드만 골라 `financial_entries`에 넣고 `created_by`를 세션 사용자로 채운다.
5. `activity_logs`에 `financial_created`를 남기고 새 행을 돌려준다. 화면은 react-query 캐시를 무효화해 목록을 다시 읽는다.

수정·삭제(`PATCH/DELETE /api/financial-entries/[id]`)는 3번까지 같고, 수정은 요청 본문 전체를 그대로 update에 넘긴다.

## 대표 흐름 — 크론

Vercel 크론이 GET으로 호출한다(스케줄은 UTC).
