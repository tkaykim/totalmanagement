...
- 전체 조회는 `src/lib/supabase/fetch-all.ts`의 `fetchAllRows` 하나로 `range`를 돌며 끝까지 읽는다. 매출·지출·프로젝트·할일 라우트가 모두 이 함수를 쓴다. 일반 직원의 보기 범위는 전 프로젝트를 페이지로 읽어 판정한 id 집합으로 거른다.
